...
comando --> [figclone], [N], [N2], [N3], {atom_number(N, Id), atom_number(N2, X), atom_number(N3, Y), figuraclone(Id, X, Y)}.
comando --> [pf], [N],  { atom_number(N, X), parafrente(X)}.
comando --> [figpf], [N1], [N2], {atom_number(N1, Id), atom_number(N2, A), figuraparafrente(Id, A)}.
comando --> [pt], [N],  { atom_number(N, X), paratras(X) }.
comando --> [figpt], [N1], [N2], {atom_number(N1, Id), atom_number(N2, A), figuraparatras(Id, A)}.
comando --> [gd], [G],  { atom_number(G, X), giradireita(X) }.
comando --> [figgd], [N1], [N2], {atom_number(N1, Id), atom_number(N2, A), figuragiradireita(Id, A)}.
comando --> [ge], [G],  { atom_number(G, X), giraesquerda(X) }.
comando --> [figge], [N1], [N2], {atom_number(N1, Id), atom_number(N2, A), figuragiraesquerda(Id, A)}.
